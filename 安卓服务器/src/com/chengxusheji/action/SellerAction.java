...
public class SellerAction extends BaseAction {
    ...
    public String QuerySeller() {
        ...
        List<Seller> sellerList = sellerDAO.QuerySellerInfo(sellUserName, sellerName, telephone, addDate, currentPage);
        /*计算总的页数和总的记录数*/
        sellerDAO.CalculateTotalPageAndRecordNumber(sellUserName, sellerName, telephone, addDate);
        /*获取到总的页码数目*/
    ...
    public String QuerySellerOutputToExcel() { 
        ...
        if(telephone == null) telephone = "";
        if(addDate == null) addDate = "";
        List<Seller> sellerList = sellerDAO.QuerySellerInfo(sellUserName,sellerName,telephone,addDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Seller信息记录"; 
        String[] headers = { "商家账号","商家名称","联系电话","入驻日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<sellerList.size();i++) {
        	Seller seller = sellerList.get(i); 
        	dataset.add(new String[]{seller.getSellUserName(),seller.getSellerName(),seller.getTelephone(),new SimpleDateFormat("yyyy-MM-dd").format(seller.getAddDate())});
        }
        /*
